<?php
/*
  Show up to 3 linecharts, boxes and table
  GET Parameters:
...
if (!isset($_GET['tab' ]))  {$_GET['tab']   = defaultTab;}        else {$_GET['tab']   = $_GET['tab'];}
...
?>
  ...
  <body>
    ...
    if ($_GET['tab'] == 1) {
      $var1Array = explode('],[', substr(substr($var1, 1), 0, -1));
      if ($varNo >= 2) {
        $var2Array = explode('],[', substr(substr($var2, 1), 0, -1));
      }
      if ($varNo >= 3) {
        $var3Array = explode('],[', substr(substr($var3, 1), 0, -1));
      }
      ...
      echo '<th>' . $dictArray[$_GET['var1']]["txtDE"] . '</th>';
      if ($varNo >= 2) {
        echo '<th>' . $dictArray[$_GET['var2']]["txtDE"] . '</th>';
      }
      if ($varNo >= 3) {
        echo '<th>' . $dictArray[$_GET['var3']]["txtDE"] . '</th>';
      }
      ...
      $date = date_create();
      $i = 0;
      for ($i = count($var1Array) - 1; $i >= 0; $i--) {
        echo '<tr>
                <td>' . date("d.m.Y     H:i:s", (int) substr($var1Array[$i], 0, strpos($var1Array[$i], ',') - 3)) . '</td>
                <td>' . substr($var1Array[$i], strpos($var1Array[$i], ',') + 1) . ' ' . $dictArray[$_GET['var1']]["Einheit"] . '</td>';
        if ($varNo >= 2) {
          echo '<td>' . substr($var2Array[$i], strpos($var2Array[$i], ',') + 1) . ' '
             . '</td>';
        }
        if ($varNo >= 3) {
          echo '<td>' . substr($var3Array[$i], strpos($var3Array[$i], ',') + 1) . ' ' . $dictArray[$_GET['var3']]["Einheit"]
             . '</td>';
        }
        echo '</tr>';
      }
